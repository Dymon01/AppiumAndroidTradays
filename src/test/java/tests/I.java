...
public class I extends BaseClass {
	...
	if (vol == "max") {
		 new TouchAction( driver).press(PointOption.point(16, 789)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
         .moveTo(PointOption.point(16, 9)).release().perform();
	} else {
		 new TouchAction( driver).press(PointOption.point(16, 550)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
         .moveTo(PointOption.point(16, 200)).release().perform();
	}

	}
//	(lower-case(@href),'logout.aspx')
	public static String request(String locator) {
		String LOCATOR = "//*[contains(@label, '" + locator + "') or contains(@name, '" + locator
				+ "') or contains(@value, '" + locator + "') or contains(@text, '" + locator + "') or contains(@resource-id, '" + locator + "') or contains(@content-desc, '" + locator + "') or contains(@type, '" + locator + "')]";
		return LOCATOR;
	}
	...
	public static String getValue(String locator, String attributte) {
		see(locator);
		String value = driver.findElement(By.xpath(request(locator))).getAttribute(attributte);;
	...
	public static String getValue(String locator) {
		...
		MobileElement element =  (MobileElement) driver
				.findElement(By.xpath(request(locator)));
...
//		WebDriverWait wait = new WebDriverWait(driver, 30);clickExecuteScript
//	wait.until(ExpectedConditions.presenceOfElementLocated(MobileBy.xpath("//*[contains(@label, '" + locator
//			+ "') or contains(@text, '" + locator + "') or contains(@value, '" + locator + "')]")));
...
//	System.out.println(elementID+"<<<<<---"+locator);
	HashMap<String, String> scrollObject = new HashMap<String, String>();
//	scrollObject.put("toVisible", "not an empty string");
	 scrollObject.put("toVisible", "true");
	scrollObject.put("element", elementID);
	// scrollObject.put("predicateString", locator);
	...
	try {
		((FindsByAndroidUIAutomator) driver).findElementByAndroidUIAutomator(
				"new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView(new UiSelector().resourceId(\""+releaseBundle +".debug:id/"+ locator + "\").instance(0))")
				.click();
	} catch (Exception e) {
	...
	((FindsByAndroidUIAutomator) driver).findElementByAndroidUIAutomator(
			"new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView(new UiSelector().descriptionContains(\"" + locator + "\").instance(0))")
			.click();
	return null;
	}
	...
	System.out.println(
			"<<<-"+locator+"- at x:"+Xoffset+" y:"+Yoffset+"||                   ");
	return null;
...
//				"~/Library/Android/sdk/platform-tools/adb -s "+device+" shell input keyevent 111" });
			}
			Thread.sleep(1500);
			try {
				Thread.sleep(1500);
			//	ClickByLocation(Locator.DONE);
			} catch (Exception e) {
				try {
